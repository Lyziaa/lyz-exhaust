...
local vehtype = GetEntityModel(veh)
	ForceVehicleEngineAudio(veh, vehtype)
	SetDisplay(false)
end)

RegisterNUICallback("exhaustchange", function(data)
local ped = PlayerPedId()
local veh = GetVehiclePedIsUsing(ped)
	ForceVehicleEngineAudio(veh, data.type)
	SetDisplay(false)
end)
